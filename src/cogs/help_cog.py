    ...
    async def help_command(self, ctx, *args):
        ...
        **Usage:**
        ...
        if not args:
                ...
                if not cog.hidden:
                    ...
                    if cog_commands:
                        command_list = [f"`{cmd.name}`" + (
                            f" (aliases: {' | '.join(cmd.aliases)})" if cmd.aliases else "") for cmd in cog_commands]
            ...
            await ctx.send(f"Invalid number of arguments. Usage: `{self.bot.command_prefix}help [command]`")
            return
